...
模型部分採用自行實作之 多層感知機（MLP，有一個隱藏層，把 MLP 隱藏層的 hiden output 數量增加(約輸入維度的 1.5 到 2 倍)，訓練次數約 10000~20000 次，學習率 0.01，就能
盡量走到終點。），僅使用 NumPy 等基礎數值運算函式庫完成前向傳播與訓練流程，未使用 TensorFlow、PyTorch 等高階 AI 框架。
